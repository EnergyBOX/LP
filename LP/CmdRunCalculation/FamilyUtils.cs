...
namespace LP
{
    public static class FamilyUtils
    {
        ...
        public static (FamilyInstance instance, List<(XYZ proposed, XYZ actual)> pointsLog) PlaceMash(
            Document doc, List<XYZ> sortedTriplet, double radius)
        {
            ...
            // Перевіряємо, що кількість точок відповідає очікуваній
            ...
            // 4. Присвоюємо координати та формуємо лог
            ...
            for (int i = 0; i < placePointIds.Count; i++)
            {
                ...
                pointsLog.Add((proposed, rp.Position));
            }
